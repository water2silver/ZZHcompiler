...
#line 137 "../Calculator.y"
    {
		...
		// 变量节点
		ast_node * id_node = new_ast_leaf_node(var_id_attr{(yyvsp[(1) - (4)].var_id).id, (yyvsp[(1) - (4)].var_id).lineno});

		free((yyvsp[(1) - (4)].var_id).id);

        // 创建一个AST_OP_ASSIGN类型的中间节点，孩子为Id和Expr($3)
        (yyval.node) = new_ast_node(ast_operator_type::AST_OP_ASSIGN, id_node, (yyvsp[(3) - (4)].node), nullptr);
    ;}
    break;
